...
(async () => {
    async function autoScroll(page){
        await page.evaluate(async () => {
            await new Promise((resolve, reject) => {
                let totalHeight = 0;
                let distance = 100;
                let count_scroll = 5; //if zero then infinity
                let counterScroll = 1;
                let timer = setInterval(() => {
                    ...
                    if ((totalHeight >= scrollHeight) || (count_scroll > 0 && counterScroll >= count_scroll)) {
                        clearInterval(timer);
                        resolve();
                    }
                }, 100);
    ...
    const browser = await puppeteer.launch({args: ['--no-sandbox', '--disable-setuid-sandbox', '--user-data-dir', '--proxy-server=socks5://127.0.0.1:9055']});
